...
class PostService {
  ...
  static async getTimeLine(object) {
    try {
      ...
      const friendPosts = await userService.getFriendsPosts({userId, limit, page})
      ...
      const currentUserAndFriendsPosts = Array.prototype.concat.apply(userPosts?.docs, friendPosts?.docs) ?? []

      const idsPosts = currentUserAndFriendsPosts.map((currentPost)=> currentPost?._id)
   
      const results = await Post.paginate({_id: {$in: idsPosts}}, options)

      const postsWithTotalPoints = await Promise.all(results?.docs?.map(async (currentPost) => {
      ...
      const orderedPosts =  postsWithTotalPoints.sort((a, b) =>   b?.points - a?.points)

...
const FindAllReactionsPost = async  (req, res)=>{
  ...
  try {
    let reactionsIds = []
    const fieldsReaction = await Post.findById(id).select(['reactions']).populate(['reactions.gusta', 'reactions.encanta', 'reactions.divierte', 'reactions.asombra', 'reactions.entristece'])
    for(key in fieldsReaction.reactions){
     ...
     const reactions = await Reaction.paginate({_id:{$in:reactionsIds.map((id)=> id._id)}}, {limit, page})

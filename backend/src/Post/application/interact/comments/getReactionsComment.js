  ...
  try {
    ...
    for(key in comment.comment.reactions){
      if(typeof comment.comment.reactions[key] === 'object') {
        reactionsIds = reactionsIds.concat(comment.comment.reactions[key])
      }
     }


     const reactions = await Reaction.paginate({_id:{$in:reactionsIds.map((id)=> id._id)}},{limit: parsedLimit, page: parsedPage})

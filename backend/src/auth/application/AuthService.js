...
module.exports = class AuthService{
    static async Register(object){
        try {
            ...
            const user = new UserModel({ username, email, password:passwordEncryped})
    ...
    static async login(object){
        try {
            ...
            if(emailExits.error)  throw new Error('email or password are not corrects')

            const correctPassword = await comparePassword(password, emailExits.password)
           
            if(!correctPassword) throw new Error('email or password are not corrects')

...
const ButtonEditPost = ({ get, post, handleClose, postId }) => {
    ...
    return (
        <button className="button-edit-post" onClick={() => handleEditPost()}>
            {isLoadingMutationEdit && !isErrorEditPost ? <SpinnerLoader /> : <p>Edit</p>}

            {!isLoadingMutationEdit && isErrorEditPost && <ErrorButton reset={reset} />}
            <svg className="svg-edit-post" viewBox="0 0 512 512">

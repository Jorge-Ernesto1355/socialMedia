...
const Votes = ({VotesActive, hideVotes}) => {
  ...
  const  allFormsCallback = useCallback(()=> ({value, name})=>{
    const updatedForm = {
      ...form,
      [name]: value
    };
  ...
  const increase = () => setNumForm((prevNumForm) => [...prevNumForm, uuidv4()])

  const arrayForms = useMemo(()=> {
    return Object.keys(form).map((property) => ({ uuid: property, text: form[property] }))
  }, [form])
      ...
      transition={{ duration: 0.2 }}>
        <div className={`poll-votes ${expand && 'expand'} `}>
         <form className="form-votes" onSubmit={(e) => submitForm(e)}>
          <div>
             {numForm.map((i, index) => (
              <Vote key={i} updateForm={allFormsCallback} votes={form} uuid={i} index={index} />
              ))}
          </div>
        </form>
        {
          !expand &&  (
            <div className='button-form-votes'>
              <button role='increase-vote'  onClick={()=>increase()}><img src={plus} alt='plus-vote'/></button>
          </div>
          )
        }
        </div>
